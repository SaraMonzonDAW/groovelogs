...
export default function ResultsList({ results, loading, searchQuery }) {
  ...
  const filteredResults = useMemo(() => {
    ...
    if (activeFilter === "track") {
      return results.filter((item) => item.type === "release");
    }
      ...
      {!filteredResults.length ? (
        ...
        <ul className="results-grid">
          {filteredResults.map((item) => (
            <AlbumCard
              key={item.id}
              searchTrack={activeFilter === "track" ? searchQuery : null}
              item={item}
